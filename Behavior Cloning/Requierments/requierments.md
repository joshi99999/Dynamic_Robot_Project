...
* **Python-Zugriff auf die Wrist-Kamera:** **Verifiziert am Gerät** (VEN-161-61U3C-M05, SN `EBK24100633`): Bildabgriff über `gxipy`, voller Sensor 1440 × 1080, 61 fps, Frame-Alter 7–13 ms; die Adapter-Contract-Tests laufen gegen die reale Kamera durch. Inbetriebnahme, Stolpersteine und Einstellungen sind in `Dokumentation/Kamera-Inbetriebnahme.md` dokumentiert, Befunde in AP 1.1.
* **Objektiv der Wrist-Kamera:** **Entschieden** — Fisheye 1,85 mm (Bildkreis 1/1.8", 12 MP). Begründung und Konsequenzen in AP 1.1.
...
1. **Zweite Kamera (Szenen-/Top-View):** Exaktes Modell noch offen. **Neue Option (bevorzugt zu prüfen): eine zweite Daheng VEN-161-61U3C mit dem zweiten (rektilinearen) Objektiv** statt einer UVC-Webcam — siehe Bewertung in AP 1.1. Solange nicht entschieden, ist `config.SCENE_CAMERA` ein Platzhalter auf OpenCV-Index 0; das ist auf einem Laptop die **eingebaute Webcam**. `apps/record.py` fragt deshalb vor jeder Aufzeichnung nach, solange `SCENE_CAMERA_CONFIRMED = False` steht.
...
**Virtuelle Steuerung — `Neurapy-OfflineSim` (angekündigt, Material folgt: Anleitung + OVA-Datei).** Neura stellt eine virtuelle Steuerung als **OVA-Appliance** bereit, läuft also in einer VM (VirtualBox/VMware). Sofern sie dieselbe NeuraPy-Schnittstelle über TCP/IP bedient, verschiebt sie einen erheblichen Teil der Abnahmeliste 0.6 nach vorn — ohne physische Anlage:
...
* **Benötigt wird dafür:** Host + Port der virtuellen Steuerung, die zugehörige NeuraPy-Version und die Info, ob Servo-Interface und Greiferbefehle dort ebenfalls bedient werden.
* **Vorab zu klärender Punkt aus dem OVA-Format:** NeuraPy verbindet sich als Client auf einen festen Socket (Default `192.168.2.13:65432`). Aus einer VM heraus ist damit die **Netzwerkkonfiguration der Appliance** der erste Stolperstein — NAT reicht nicht, es braucht Host-Only- oder Bridged-Networking mit erreichbarer IP. Der Adapter unterstützt das bereits, weil `NeuraRobot` eine fertige `Robot`-Instanz entgegennehmen kann; ein Host/Port-Parameter ist dafür nicht nötig.
...
* **Inbetriebnahme-Befunde Wrist-Kamera (gemessen am Gerät, VEN-161-61U3C-M05, SN EBK24100633):**
  * **Python-Zugriff steht.** `gxipy` liegt beim Galaxy SDK nur als Ordner unter `…/GalaxySDK/Development/Samples/Python/gxipy` und ist **nicht** installierbar (kein `setup.py`, kein PyPI-Paket). Der Adapter sucht den Pfad deshalb zur Laufzeit; überschreibbar über die Umgebungsvariable `GALAXY_SDK_PYTHON`. Eine Kopie des Vendor-Codes im Repo wurde bewusst vermieden — sie würde bei SDK-Updates still veralten.
  * **Stolperstein Umgebungsvariablen:** Der Installer setzt `GALAXY_GENICAM_ROOT` und die DLL-Pfade nur auf **Machine**-Ebene. Windows vererbt die an bereits laufende Shells nicht, was sich als `KeyError: 'GALAXY_GENICAM_ROOT'` bzw. `Cannot find GxIAPI.dll` äußert — bei korrekter Installation. Der Adapter zieht die Werte jetzt aus der Registry nach.
  * **⚠️ Der ROI wird in der Kamera persistent gespeichert.** Wird er nicht aktiv gesetzt, liefert die Kamera stumm den zuletzt konfigurierten Ausschnitt — beobachtet: 640 × 480 aus einem 1440 × 1080-Sensor, also ein **Eckausschnitt ohne optisches Zentrum**, was beim Fisheye den größten Teil des Sichtfelds kostet. Der Adapter setzt `Width`/`Height` deshalb immer explizit (voller Sensor: `WidthMax`/`HeightMax`, Offsets 0).
  * **Binning wird von diesem Modell nicht unterstützt** (`BinningHorizontal` nicht schreibbar). Die geplante Datenreduktion entfällt damit; es bleibt beim vollen Frame. Unkritisch: 1440 × 1080 Bayer8 bei 15 Hz ≈ 23 MB/s.
  * **Gemessen (Wrist allein):** native Rate 61,3 fps, Frame-Alter 4–9 ms, Pixelformat BayerRG8, Debayering über das SDK nach RGB funktioniert, Skalierung auf die Schemagröße 240 × 320 durchgängig.
  * *Kein Zeitversatz-Messwert vorhanden.* Eine erste Messung lief gegen die eingebaute Laptop-Webcam und ist damit für das Projekt **nicht aussagekräftig**; die Szenenkamera war nicht angeschlossen. Der Zeitversatz nach 1.3 ist erst mit beiden Projektkameras zu messen.
  * **⚠️ Geräteauswahl per Seriennummer:** Sobald zwei Daheng-Kameras angeschlossen sind, ist die Index-Reihenfolge nicht stabil (Enumerationsreihenfolge, USB-Port, Einschaltzeitpunkt). Wrist- und Szenenbild könnten vertauscht im Datensatz landen — ein Fehler, der beim Training nicht auffällt. Der Adapter verlangt daher bei mehreren Geräten zwingend eine Seriennummer. Wrist: `EBK24100633`.
  * **⚠️ Offen vor der Datenaufzeichnung — Auto-Regelungen festnageln:** `BalanceWhiteAuto` kann bei diesem Modell nur `Off`/`Once`, und `Once` hängt davon ab, was beim Programmstart im Bild war — die Farbstatistik schwankt damit **zwischen Sessions**. Dasselbe gilt für `GainAuto` (gemessen: 16,6 dB automatisch geregelt). Beides ändert die Bildstatistik, auf die die Policy trainiert wird. Vor der echten Aufzeichnung sind feste Weißabgleich-Ratios und ein fester Gain zu messen und in `config.WRIST_CAMERA` zu pinnen.
...
* **Hardware-Trigger**
  Die Daheng-Kamera unterstützt Hardware- und Software-Trigger (opto-isolierter Eingang Line0 + programmierbarer GPIO). Eine echte getriggerte Synchronisation beider Kameras ist **nur möglich, wenn auch die Szenenkamera triggerfähig ist** — bei einer UVC-Webcam ist das nicht der Fall.
  **Neu zu bewerten, falls die Szenenkamera ebenfalls eine Daheng wird (siehe Statusliste oben):** Dann sind beide Kameras triggerfähig, und das zeitstempelbasierte Sampling aus 1.3 ließe sich durch eine **echte getriggerte Synchronisation** ersetzen. Das würde die größte verbleibende Unsicherheit der Datenqualität beseitigen — der Zeitversatz zwischen den Bildquellen wäre dann hardwareseitig ≈ 0 statt „innerhalb eines 30-ms-Budgets". Die Timestamp-Synchronisation bleibt trotzdem nötig, weil der **Roboterzustand** ohnehin nur zeitstempelbasiert einzuordnen ist; der Nutzen liegt in der Bild-zu-Bild-Konsistenz.

* **Bewertung: zweite Daheng statt UVC-Webcam für die Top-View**
  Spricht dafür:
  * **Identische Bildkette** — gleicher Sensor, gleiches Debayering, gleiche Vorverarbeitung. Damit entfällt eine ganze Klasse von Inkonsistenzen zwischen den beiden Strömen.
  * **Steuerbarkeit der Auto-Regelungen.** Belichtung, Gain und Weißabgleich lassen sich fest pinnen. Bei UVC-Webcams ist das oft nicht oder nur unzuverlässig möglich — und eine nachregelnde Automatik verändert die Bildstatistik, auf die trainiert wird (siehe Inbetriebnahme-Befunde).
  * **Global Shutter auch für die Top-View** — relevant für das Hand-Szenario aus AP 5.1, wo sich Objekte schnell durchs Bild bewegen.
  * **Hardware-Trigger** (siehe oben).
  * Das rektilineare Objektiv passt zur Top-View besser als zur Wrist-Kamera: Die Deckenkamera hat einen weitgehend **konstanten Arbeitsabstand**, womit das Festfokus-Problem aus der Objektivbewertung entfällt. Zusätzlich ist die ArUco-Homographie-Rektifizierung (1.4) auf einer rektilinearen Optik deutlich besser konditioniert als auf einem Fisheye.
  Zu beachten:
  * **USB3-Bandbreite.** Zwei VEN-161 mit nativen 61 fps und vollem Sensor ergäben zusammen rund 190 MB/s. Das passt zwar nominell auf einen USB3-Controller, ist aber knapp — insbesondere, wenn beide am selben Root-Hub hängen. Da nur mit 15 Hz abgetastet wird, ist die Rate zu begrenzen (Richtwert 30 fps je Kamera, ausreichend Reserve im Latenzbudget) bzw. `DeviceLinkThroughputLimit` zu setzen. Nach Möglichkeit auf getrennte Controller verteilen.
  * **Eindeutige Zuordnung per Seriennummer** ist dann Pflicht (siehe Inbetriebnahme-Befunde).
  * **Auswirkung auf den klassischen CV-Ablauf (AP 5.1):** Der Wegfall von `cv2.VideoCapture` ist der einzige erkennbare Nachteil und betrifft nur den Bildabgriff, nicht die Verarbeitung — `gxipy` liefert NumPy-Arrays, ab dem Frame ist die bestehende CV-Pipeline unverändert nutzbar. Der Adapter (`bc/adapters/cam_daheng.py`) existiert bereits und kann vom CV-Team mitverwendet werden. Zusätzlicher Nutzen für die spätere Datenübernahme: gleiche Kamera und gleiche Vorverarbeitung sind laut 5.1 ohnehin **Voraussetzung** dafür, die CV-Demonstrationen ohne Nacharbeit ins Training zu übernehmen — eine zweite Daheng erfüllt diese Bedingung automatisch, eine abweichende Webcam nicht.
